...
import { fetchHosts, blockHost, unblockHost, deleteHost } from "../../../redux/slices/hostSlice/hostSlice";
...
interface HostUserProps {
  onNavigate: (page: string, data?: Host) => void;
}

export default function HostUser({ onNavigate }: HostUserProps) {
  ...
  const [modalType, setModalType] = useState<'block' | 'unblock' | 'delete'>('block');
  ...
  useEffect(() => {
    dispatch(fetchHosts({ page, limit, sort: sortValue, status: statusValue, search: debouncedSearch }));
  ...
  const handleBlock = (row: Host) => {
    setModalType('block');
    setCurrentRow(row);
    setIsModalOpen(true);
  };

  const handleUnblock = (row: Host) => {
  ...
  const submitBlock = async () => {
    ...
    try {
      await dispatch(blockHost({ hostId: currentRow.hostID, blockReason: reason })).unwrap();
  ...
  const submitUnblock = async () => {
    if (!currentRow) return;
    try {
      await dispatch(unblockHost(currentRow.hostID)).unwrap();
      showSuccessAlert('Success', 'Host unblocked successfully');
      setIsModalOpen(false);
      setCurrentRow(null);
      dispatch(fetchHosts({ page, limit, sort: sortValue, status: statusValue, search: debouncedSearch }));
    } catch (error) {
  ...
  const submitDelete = async () => {
    if (!currentRow) return;
    try {
      await dispatch(deleteHost(currentRow.hostID)).unwrap();
      showSuccessAlert('Success', 'Host deleted successfully');
      setIsModalOpen(false);
      setCurrentRow(null);
      dispatch(fetchHosts({ page, limit, sort: sortValue, status: statusValue, search: debouncedSearch }));
    } catch (error) {
  ...
  const renderActions = (row: Host) => {
    ...
    const buttons = isBlocked
      ? [
          {
            label: "Unblock",
            variant: "unblock" as const,
            onClick: (row: Host) => handleUnblock(row),
          },
          {
            label: "Delete",
            variant: "delete" as const,
            onClick: (row: Host) => handleDelete(row),
          },
        ]
      : [
          {
            ...
            onClick: (row: Host) => handleBlock(row),
          },
          {
            label: "Delete",
            variant: "delete" as const,
  ...
  return (
    <div className="px-4 py-8">
     <div className="flex justify-between items-center">
      <h1 className="text-2xl font-semibold text-gray-900">Host List and Details</h1>
     ...
     <div className="mt-8">
       {isLoading ? (
         ...
         <ReusableTable
           ...
           onRowClick={(row) => onNavigate("host-details", row)}
         />
       )}
     </div>
     <BasicModal
       ...
       title={
         modalType === 'block' ? 'Block reason' :
         modalType === 'unblock' ? 'Unblock Host' :
         'Delete Host'
       }
       modalContent={
         modalType === 'block' ? (
           ...
           <div className="p-4">
             ...
             <div className="flex gap-2">
               <Button
                 variant="primary"
                 label={modalType === 'unblock' ? 'Unblock' : 'Delete'}
                 onClick={modalType === 'unblock' ? submitUnblock : submitDelete}
                 width="w-full"
               />
               <Button
                 variant="secondary"
